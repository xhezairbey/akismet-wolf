  ...
  */
  function spamCheck(&$comment) {
	...
	function ham($id) {
      ...
      if ($comment->save()) {
        Flash::set('success', __('Spam comment has been marked as ham!'));
        Observer::notify('spam_comment_marked_ham', $comment);
      }
      else
        Flash::set('error', __('Oops! Something went wrong!'));
    }
    else
      Flash::set('error', __('Spam comment not found!'));
    ...
    if ($comment = Record::findByIdFrom('Comment', $id)) {
      ...
      if ($comment = Record::findByIdFrom('Comment', $id))
      {
        ...
        if ($comment->save()) {
          Flash::set('success', __('Comment has been marked as spam!'));
          Observer::notify('comment_after_spam', $comment);
        }
      }
